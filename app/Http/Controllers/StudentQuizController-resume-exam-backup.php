...
class StudentQuizController extends Controller
{
    ...
    public function exams($slug='')
    {
      ...
      if ($slug)
      {
          ...
          else
          {
              ...
              {
                 $child = User::select(['settings'])->where('parent_id', '=', $user->id)->get()->first();
    ...
    public function startExam($slug)
    {
        ...
        {
          $temp_prepared_records = App\QuizQuestions::where('student_id','=',$user->id) 
                                                 ->where('quiz_id','=',$quiz->id)
                                                 ->where('is_exam_completed',0)
                                                 ->first();
         

            $questions_data = json_decode($temp_prepared_records->questions_data);
            $questions = [];
            $subjects = [];
            foreach($questions_data as $question)
            {

              $questionb    = QuestionBank::where('id','=',$question->id)->first();
              $subjectb     = Subject::where('id','=',$question->subject_id)->first();
              $questions[]  = $questionb;
              $subjects[]   = $subjectb;

            }

          $prepared_records = (object)(['questions'=>$questions, 'subjects'=>$subjects]);
          if($temp_prepared_records->current_state) 
          {   

            $current_state       = (array) json_decode($temp_prepared_records->current_state);
            $current_question_id = $temp_prepared_records->current_question_id;
            $time['hours']       = $temp_prepared_records->current_hour;
            $time['minutes']     = $temp_prepared_records->current_minute;
            $time['seconds']     = $temp_prepared_records->current_second;
        ...
        $resume_data        = null;//json_decode($exam->current_state);
        $resume_exam_data   = null;// json_decode($resume_data->exam_data);
        $resume_rigth_data  = null; //json_decode($resume_data->right_bar_data);
        $resume_left_data   = null; //json_decode($resume_data->left_bar_data);
        $present_question   = null;//json_decode($resume_data->present_question);
        // dd($present_question);
        $data['resume_exam_data']  = $resume_exam_data;
        $data['resume_rigth_data'] = $resume_rigth_data;
        $data['resume_left_data']  = $resume_left_data;
        $data['present_question']  = $present_question;
    ...
    public function finishExam(Request $request, $slug)
    {
      ...
      // $user_record->notify(new \App\Notifications\UsersNotifcations($user_record,$content_data)); 
